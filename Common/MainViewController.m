...
//    __weak CallViewController *_callController;
    AudioVisualViewController *_visualVC;
    ReadViewController *_readVC;
...
- (void)tabBar:(UITabBar *)tabBar didSelectItem:(UITabBarItem *)item
{
    if (item.tag == 0) {
        ...
        self.title = NSLocalizedString(@"title.reading", @"Reading");
        self.navigationItem.rightBarButtonItem = nil;
        [_settingsVC refreshConfig];
    }
    else if (item.tag == 4){
        self.title = NSLocalizedString(@"title.visual", @"Visual");
        self.navigationItem.rightBarButtonItem = nil;
        [_settingsVC refreshConfig];
    }
}
...
//    阅读页面
    ...
    _readVC.tabBarItem = [[UITabBarItem alloc] initWithTitle:NSLocalizedString(@"title.reading", @"Reading")
                                                         image:nil
                                                           tag:3];
    [_readVC.tabBarItem setFinishedSelectedImage:[UIImage imageNamed:@"tabbar_reading"]
                       withFinishedUnselectedImage:[UIImage imageNamed:@"tabbar_reading"]];
    _readVC.view.autoresizingMask = UIViewAutoresizingFlexibleHeight;
    [self unSelectedTapTabBarItems:_readVC.tabBarItem];
    [self selectedTapTabBarItems:_readVC.tabBarItem];
   
    //    视听页面
    _visualVC = [[AudioVisualViewController alloc] init];
    UINavigationController *visual_nav = [[UINavigationController alloc]initWithRootViewController:_visualVC];
    _visualVC.tabBarItem = [[UITabBarItem alloc] initWithTitle:NSLocalizedString(@"title.visual", @"visual")
                                                         image:nil
                                                           tag:4];
    [_visualVC.tabBarItem setFinishedSelectedImage:[UIImage imageNamed:@"tabbar_visual"]
                       withFinishedUnselectedImage:[UIImage imageNamed:@"tabbar_visual"]];
    _visualVC.view.autoresizingMask = UIViewAutoresizingFlexibleHeight;
    [self unSelectedTapTabBarItems:_visualVC.tabBarItem];
    [self selectedTapTabBarItems:_visualVC.tabBarItem];
    
    self.viewControllers = @[_chatListVC, _contactsVC, _readVC,visual_nav,_settingsVC];
    [self selectedTapTabBarItems:_chatListVC.tabBarItem];
}
